...
This is part of the [Further Enterprise Application Architecture development](/eaaDev) writing that I was doing in the mid 2000âs. Sadly too many other things have claimed my attention since, so I havenât had time to work on them further, nor do I see much time in the foreseeable future. As such this material is very much in draft form and I wonât be doing any corrections or updates until Iâm able to find time to work on it again.
...
logic in [Transaction Scripts](/eaaCatalog/transactionScript.html) or [Domain Model](/eaaCatalog/domainModel.html). As usual [Transaction Scripts](/eaaCatalog/transactionScript.html) are better for simple
logic and a [Domain Model](/eaaCatalog/domainModel.html) is better when things get more
complicated.

In general I have noticed a tendency to use [Transaction Scripts](/eaaCatalog/transactionScript.html) with applications that
...
You can combine these together, essentially this is the [Transaction Script](/eaaCatalog/transactionScript.html) approach, but you can also separate
...
object is a [DTO](/eaaCatalog/dataTransferObject.html) which is
...
[DTO](/eaaCatalog/dataTransferObject.html) as an hidden data holder
...
make a [Domain Model](/eaaCatalog/domainModel.html) ignorant of the event
log. Reversing logic makes this more tricky since the
[Domain Model](/eaaCatalog/domainModel.html) needs to store and retrieve the
prior state, which makes it much more handy for the
[Domain Model](/eaaCatalog/domainModel.html) to be aware of the event log.
...
in the form of a difference. An example of this would be âadd $10 to
Martin's accountâ as opposed to âset Martin's account to $110â. In
...
a [Gateway](/eaaCatalog/gateway.html). This in itself isn't too onerous
...
[Retroactive Event](RetroactiveEvent.html) see the discussion there
for gory details.
...
reliable, then we can use that to ensure consistent replay. It also may be that we are using [Event Collaboration](EventCollaboration.html), in which case all we have to ensure we retain the history of changes.
...
what needs to happen with [Retroactive Events](RetroactiveEvent.html). Indeed if there's
a lot of reprocessing to consider it would be worth actually
using the [Retroactive Event](RetroactiveEvent.html) mechanism to
...
rule along the lines of âcharge $10 before November 18 and $15
afterwordsâ. This kind of stuff needs to actually go into the domain
...
[Temporal Property](TemporalProperty.html): something like
`chargingRules.get(aDate).process(anEvent)`. Take a look at
[Agreement Dispatcher](AgreementDispatcher.html) for this kind of style.
...
behavior: âreverse this event according to the rules for Aug 1
that we had on Oct 1 and replace it according to the rules for
Aug 1 that we have nowâ. Clearly this stuff can get very messy,
...
[Domain Event](DomainEvent.html) are the creation of [Accounting Entrys](AccountingEntry.html) and link these entries to
...
[Accounting Entrys](AccountingEntry.html) are a log of
all of the events that change the value of an [Account](Account.html), so that an [Account](Account.html) is
...
events to make an [Audit Log](AuditLog.html).Such an audit
...
Another use for this kind of complete [Audit Log](AuditLog.html) is to help with debugging. Of course, it's old hat
...
Event Sourcing is the foundation for [Parallel Models](ParallelModel.html) or [Retroactive Events](RetroactiveEvent.html). If you want to use either of those patterns
...
A departure event just sets the Ship's port to a [Special Case](/eaaCatalog/specialCase.html). You'll notice I'm passing the event into the
...
with lots of âehâ. I've seen books where nearly every sentence has
...
through a [Gateway](/eaaCatalog/gateway.html) that I control. The basic
...
items by using the [Domain Event](DomainEvent.html) that was
...
as the log of [Domain Events](DomainEvent.html) is
